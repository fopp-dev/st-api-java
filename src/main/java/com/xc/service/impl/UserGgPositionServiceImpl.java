...
@Service("iUserGgPositionService")
public class UserGgPositionServiceImpl implements IUserGgPositionService {

    private static final Logger log = LoggerFactory.getLogger(UserGgPositionServiceImpl.class);

    @Autowired
    UserGgPositionMapper userPositionMapper;

    @Autowired
    IUserService iUserService;

    @Autowired
    ISiteHksSettingService iSiteSettingService;
    ...
    @Autowired
    SiteHksSettingServiceImpl siteHksSettingService;
    ...
    public ServerResponse buy(Integer stockId, Integer buyNum, Integer buyType, Integer lever, HttpServletRequest request) throws Exception {
        ...
        c.setTime(today);
        int weekday = c.get(Calendar.DAY_OF_WEEK);// todo
//        if (weekday == 1) {
...
//        }
        ...
        BigDecimal user_enable_amt = user.getEnableHmt();
        ...
        SiteHksSetting siteSetting = this.iSiteSettingService.getSiteHksSetting();
        if (siteSetting == null) {
...
//        }
        ...
        StockGg stock = null;
        ServerResponse stock_res = this.iStockService.findStockGgById(stockId);
        if (!stock_res.isSuccess()) {
            return ServerResponse.createByErrorMsg("下单失败，股票代码错误");
        }
        stock = (StockGg) stock_res.getData();
...
//        BigDecimal now_price = new BigDecimal(stockListVO.getNowPrice());
        ServerResponse ggSingleStock = stockService.getGGSingleStock(stock.getStockCode());
        ...
        BigDecimal bhcrate = new BigDecimal(hcrate);
        ...
        double stock_crease = bhcrate.doubleValue();


        BigDecimal maxRisePercent = new BigDecimal("0.1");
...
//            log.info("【普通A股】");
//        }
        BigDecimal zsPrice = new BigDecimal(String.valueOf(result.get("preclose_px")));
        ...
        ServerResponse serverResponse = this.iStockService.selectRateByDaysAndStockGgCode(stock
        ...
        UserGgPosition userPosition = new UserGgPosition();
        ...
        if (insertPositionCount > 0) {
            ...
            user.setEnableHmt(reckon_enable);
    ...
    public ServerResponse sell(String positionSn, int doType) throws Exception {
        ...
        UserGgPosition userPosition = this.userPositionMapper.findPositionBySn(positionSn);
        ...
        //通过API获取港股信息
        ServerResponse ggSingleStock = stockService.getGGSingleStock(userPosition.getStockCode());
        com.alibaba.fastjson.JSONObject result = (com.alibaba.fastjson.JSONObject)ggSingleStock.getData();
        String nowPrice = String.valueOf(result.get("nowPrice"));
        String hcrate = String.valueOf(result.get("hcrate"));
        BigDecimal now_price = new BigDecimal(nowPrice);
        ...
        BigDecimal user_all_amt = user.getUserHmt();
        BigDecimal user_enable_amt = user.getEnableHmt();
        ...
        BigDecimal reckon_enable = user_enable_amt.add(all_profit).add(all_buy_amt);
//        BigDecimal reckon_enable = user_enable_amt.add(all_profit);
        log.info("用户平仓后的总资金  = {} , 可用资金 = {}", reckon_all, reckon_enable);
        user.setUserHmt(reckon_all);
        user.setEnableHmt(reckon_enable);

        BigDecimal profit = user.getUserStockHKCapital().add(all_profit);

        user.setUserStockHKCapital(profit);
    ...
    public ServerResponse lock(Integer positionId, Integer state, String lockMsg) {
        ...
        UserGgPosition position = this.userPositionMapper.selectByPrimaryKey(positionId);
        if (position == null) {
    ...
    public ServerResponse findMyPositionByCodeAndSpell(String stockCode, String stockSpell, Integer state, HttpServletRequest request, int pageNum, int pageSize) {
        ...
        List<UserGgPosition> userPositions = this.userPositionMapper.findMyPositionByCodeAndSpell(user.getId(), stockCode, stockSpell, state);
    ...
    public PositionVO findUserPositionAllProfitAndLose(Integer userId) {
        List<UserGgPosition> userPositions = this.userPositionMapper.findPositionByUserIdAndSellIdIsNull(userId);

        List<UserGgPosition> userPositionsNo = this.userPositionMapper.findPositionByUserIdAndSellIdIsNotNull(userId);
        ...
        SiteHksSetting siteSetting = siteHksSettingService.getSiteHksSetting();
        for (UserGgPosition position : userPositions) {

            ServerResponse stockhjson = stockService.getGGSingleStock(position.getStockCode());
            JSONObject data = (JSONObject) stockhjson.getData();

            BigDecimal nowPrice = data.getBigDecimal("nowPrice");
...
//                allFree = allFree.add(total_fee1);
        ...
        for (UserGgPosition position : userPositionsNo) {
                BigDecimal total_fee1 = position.getOrderFee().add(position.getOrderSpread()).add(position.getOrderStayFee()).add(position.getSpreadRatePrice());
                allFree = allFree.add(total_fee1);
        }
        //加上分仓交易保证金
    ...
    public List<UserGgPosition> findPositionByUserIdAndSellIdIsNull(Integer userId) {
        return this.userPositionMapper.findPositionByUserIdAndSellIdIsNull(userId);
    }

    public List<UserGgPosition> findPositionByStockCodeAndTimes(int minuteTimes, String stockCode, Integer userId) {
    ...
    public ServerResponse listByAgent(Integer positionType, Integer state, Integer userId, Integer agentId, String positionSn, String beginTime, String endTime, HttpServletRequest request, int pageNum, int pageSize) {
        ...
        List<UserGgPosition> userPositions = this.userPositionMapper.listByAgent(positionType, state, userId, searchId, positionSn, begin_time, end_time);
    ...
    public ServerResponse getIncome(Integer agentId, Integer positionType, String beginTime, String endTime) {
        ...
        List<UserGgPosition> userPositions = this.userPositionMapper.listByAgent(positionType, Integer.valueOf(1), null, agentId, null, begin_time, end_time);
    ...
    public ServerResponse listByAdmin(Integer agentId, Integer positionType, Integer state, Integer userId, String positionSn, String beginTime, String endTime, int pageNum, int pageSize) {
        ...
        List<UserGgPosition> userPositions = this.userPositionMapper.listByAgent(positionType, state, userId, agentId, positionSn, begin_time, end_time);
    ...
    public ServerResponse create(Integer userId, String stockCode, String buyPrice, String buyTime, Integer buyNum, Integer buyType, Integer lever) {
        ...
        StockGg stock = (StockGg) this.iStockService.findStockGgByCode(stockCode).getData();
        if (stock == null) {
    ...
    public void doClosingStayTask() {
        List<UserGgPosition> userPositions = this.userPositionMapper.findAllStayPosition();
    ...
    @Transactional
    public ServerResponse closingStayTask(UserGgPosition position, Integer stayDays) throws Exception {
    ...
    private AdminPositionVO assembleAdminPositionVO(UserGgPosition position) {
    ...
    private AgentPositionVO assembleAgentPositionVO(UserGgPosition position) {
    ...
    private UserPositionVO assembleUserPositionVO(UserGgPosition position) {
    ...
    private PositionProfitVO getPositionProfitVO(UserGgPosition position) {
        ...
        if (position.getSellOrderId() != null) {
            ...
            ServerResponse ggSingleStock = stockService.getGGSingleStock(position.getStockCode());
            com.alibaba.fastjson.JSONObject result = (com.alibaba.fastjson.JSONObject)ggSingleStock.getData();
            if(result == null)
                nowPrice = "0";
            else
                nowPrice = String.valueOf(result.get("nowPrice"));
    ...
    public ServerResponse findPositionTopList(Integer pageSize) {
        List<UserGgPosition> userPositions = this.userPositionMapper.findPositionTopList(pageSize);
    ...
    public ServerResponse findUserPositionByCode(HttpServletRequest request, String stockCode) {
        User user = this.iUserService.getCurrentRefreshUser(request);
        UserGgPosition position = this.userPositionMapper.findUserPositionByCode(user.getId(), stockCode);
