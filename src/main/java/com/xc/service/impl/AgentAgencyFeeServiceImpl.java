    ...
    * */
    ...
    public int AgencyFeeIncome(int feeType,String positionSn) {
        ...
        if(userPosition == null ){
            UserGgPosition userGgPosition = userGgPositionMapper.findPositionBySn(positionSn);
            agentUser = agentUserMapper.findAgentByAgentId(userGgPosition.getAgentId());
            businessId = userGgPosition.getId();
            if(feeType == 1){
                totalMoney = userGgPosition.getOrderFee();
                remarks = "【入仓收入】入仓手续费总额："+userGgPosition.getOrderFee()+"，单号："+userGgPosition.getPositionSn();
            } else if(feeType == 2){
                //平仓浮动总盈亏负数，代理利润增加，否则减少
                totalMoney = userGgPosition.getAllProfitAndLose().multiply(new BigDecimal(-1));
                remarks = "【平仓收入】平仓手续费总额："+userGgPosition.getOrderFee()+"，单号："+userGgPosition.getPositionSn();
            } else if(feeType == 3){
                totalMoney = userGgPosition.getOrderStayFee();
                remarks = "【递延费收入】递延费总额："+userGgPosition.getOrderFee()+"，单号："+userGgPosition.getPositionSn();
            }else if(feeType == 4){
                totalMoney = userGgPosition.getAllProfitAndLose().multiply(new BigDecimal(-1));
                remarks = "【分红收入】分红总额："+userGgPosition.getOrderFee()+"，单号："+userGgPosition.getPositionSn();
            }
            //金额为0不计算分红直接跳出
            ...
            //模拟用户下单不计算分红
            User user = userMapper.selectByPrimaryKey(userGgPosition.getUserId());
