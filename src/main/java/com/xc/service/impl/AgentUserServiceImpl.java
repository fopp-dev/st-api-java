...
//        }
    ...
    public ServerResponse add(AgentUser agentUser, HttpServletRequest request) {
        ...
        AgentUser parentAgent = new AgentUser();
        if (agentUser.getParentId() != null) {
            parentAgent = this.agentUserMapper.selectByPrimaryKey(agentUser.getParentId());
            if (parentAgent != null) {
                dbAgent.setParentId(parentAgent.getId());
                dbAgent.setParentName(parentAgent.getAgentName());
                if(parentAgent.getAgentLevel() == null){
                    dbAgent.setAgentLevel(1);
                }else {
        ...
        if (insertCount > 0) {
            ...
            if(dbAgent.getId()>0) {
                if (parentAgent != null) {
                    recursiveSaveAgentDistributionUser(dbAgent.getId(), dbAgent.getParentId(), parentAgent.getAgentLevel());
    ...
    public ServerResponse update(AgentUser agentUser) {
        ...
        if (StringUtils.isNotBlank(agentUser.getAgentPhone())) {
            ...
            if (phoneAgent == null || phoneAgent.getId() == agentUser.getId()) {
                ...
                return ServerResponse.createByErrorMsg("邮箱存在，请更换邮箱");
            }
        }
    ...
    private AgentInfoVO assembleAgentInfoVO(AgentUser agentUser, String host) {
        ...
        String pcUrl =  PropertiesUtil.getProperty("site.pc.reg.url") + agentUser.getAgentCode();
        agentInfoVO.setPcUrl(pcUrl);
        String mUrl =  PropertiesUtil.getProperty("site.m.reg.url") + agentUser.getAgentCode();
